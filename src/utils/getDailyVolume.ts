 ...
 */
export default async function getDailyVolume(token: Token): Promise<CurrencyAmount<Token>> {
  ...
  try {
    ...
    const priceInUSD = pairs.find((pair: any) => pair.pairAddress.toLowerCase() === pricePair.pairAddress.toLowerCase())
      ?.priceUsd as number;
    console.log('priceInUSD', priceInUSD);
    console.log('dailyVolume', dailyVolumeInUSD / priceInUSD);

    return CurrencyAmount.fromRawAmount(token, Math.round(dailyVolumeInUSD / priceInUSD) * 10 ** token.decimals);
  } catch (error) {

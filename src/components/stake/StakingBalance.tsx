...
export function StakingBalance() {
  ...
  const token = chainId ? APPTOKEN[chainId as ChainIdValue] : undefined;
  ...
  useEffect(() => {
    let mounted = true;
    if (token) {
      getDailyVolume(token).then((volume) => {
        if (mounted) {
  ...
  const xDiffContractBalance = useTokenBalance(chainId ? XAPPTOKEN[chainId as ChainIdValue].address : undefined, token);
  ...
  const apr =
    emission && xDiffContractBalance?.toExact() !== '0'
      ? emission
            ...
            <AutoColumn justify={'end'}>
              <TYPE.largeHeader color={'primary1'} marginBottom={`10px`}>
                <TokenLogo>
                  ...
                  <QuestionHelper text={`Unstaking 1 xBROWN will earn ${ratio?.toSignificant()} BROWN`} />
                </TokenLogo>
              </TYPE.largeHeader>
            </AutoColumn>
          </RowBetween>
              ...
              <TokenLogo>
                ...
                <QuestionHelper
                  text={`${earnedDiff?.toFixed(
                    2,
                  )} BROWN is available upon unstaking ${xdiffBalance?.toSignificant()} xBROWN.`}
                />
              </TokenLogo>
            </AutoColumn>

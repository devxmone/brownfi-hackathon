...
const StyledTitleText = styled.div<{ active: boolean }>`
  ...
  font-weight: 600;
  color: ${({ theme, active }) => (active ? theme.primary1 : theme.primary1)};
`;
...
const RowWrapper = styled(Row)<{ bgColor: string; active: boolean }>`
  ...
  padding: 1rem;
  border-radius: 23px;
  border: 1px solid ${({ active, theme }) => (active ? '#F25A3B' ?? 'transparent' : theme.bg0)};
  color: ${({ active }) => (active ? '#F25A3B' : 'white')};
`;
        ...
        <RowFixed mt='4px'>
          ...
          <StyledMenu ref={node as any}>
            <ButtonEmpty onClick={toggle} ref={setReferenceElement} padding='0'>
              <Settings stroke={theme.primary1} size={12} />
            </ButtonEmpty>
            {open && (
    ...
    <Wrapper>
      ...
      {tempList && (
        <PaddedColumn style={{ paddingTop: 0 }}>
          <Card backgroundColor={theme.primary1} padding='12px 20px'>
            <RowBetween>
              <RowFixed>
              ...
              {isImported ? (
                <RowFixed>
                  <IconWrapper stroke={theme.primary1} size='16px' marginRight={'10px'}>
                    <CheckCircle />
                  </IconWrapper>
                  <TYPE.body color={theme.primary1}>Loaded</TYPE.body>
                </RowFixed>
              ) : (
                <ButtonPrimary
                  style={{ fontSize: '14px' }}

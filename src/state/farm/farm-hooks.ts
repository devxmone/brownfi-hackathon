...
import { APPTOKEN } from 'constants/tokens';
    ...
    return poolIndizes
      .map((poolId, idx) => {
        ...
        const poolEmissionAmount = chainId
          ? CurrencyAmount.fromRawAmount(APPTOKEN[chainId as ChainIdValue], poolEmissionPerSecond || 0)
          : undefined;
        ...
        const rawInfo: MinichefRawPoolInfo = {
          ...
          pendingAmount: chainId
            ? CurrencyAmount.fromRawAmount(
                APPTOKEN[chainId as ChainIdValue],
                JSBI.BigInt(pendingResult?.pending?.toString() || 0),
              )
            : undefined,
...
export function usePool(poolId: number) {
  ...
  const totalAllocation = totalAllocationResponse.result?.[0] as BigNumber | undefined;
  const pendingDiffusions = useSingleCallResult(minichefContract, 'pendingPrimaryToken', [
    poolId,
    account ?? undefined,
  ]);

...
import { SWAP_PAIR_INTERFACE } from 'state/fees/hooks';
...
import { APPTOKEN, WETH } from 'constants/tokens';
...
import { abi as TOKEN_BAR_ABI } from 'abis/DiffusionBar.json';
...
export function useContract<T extends Contract = Contract>(
  ...
  return useMemo(() => {
    ...
    else address = addressOrAddressMap[chainId as ChainIdValue];
...
export function useWETHContract(withSignerIfPossible?: boolean) {
  ...
  return useContract<Weth>(
    chainId ? WETH[chainId as ChainIdValue]?.address : undefined,
    WETH_ABI,
...
export function usePairContract(pairAddress?: string, withSignerIfPossible?: boolean): Contract | null {
  return useContract(pairAddress, SWAP_PAIR_INTERFACE, withSignerIfPossible);
}
...
export function useUniContract() {
  const { chainId } = useActiveWeb3React();
  return useContract(chainId ? APPTOKEN[chainId as ChainIdValue]?.address : undefined, UNI_ABI, true);
}
...
export function useDiffusionBar() {
  return useContract<DiffusionBar>(TOKEN_BAR_ADDRESS, TOKEN_BAR_ABI, true);
}

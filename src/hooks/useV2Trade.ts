...
import { TradeType } from 'sdk-core/constants';
import { Currency, CurrencyAmount } from 'sdk-core/entities';
import { Pair, Trade } from 'v2-sdk/entities';
...
export function useV2TradeExactOut(
  ...
  return useMemo(() => {
    if (currencyIn && currencyAmountOut && allowedPairs.length > 0) {
      ...
      for (let i = 1; i <= maxHops; i++) {
        const bestTrades = Trade.bestTradeExactOut(allowedPairs, currencyIn, currencyAmountOut, {
          maxHops: i,
          maxNumResults: 1,
        });
        const currentTrade = bestTrades[0] ?? null;
        if (isTradeBetter(bestTradeSoFar, currentTrade, BETTER_TRADE_LESS_HOPS_THRESHOLD)) {
          bestTradeSoFar = currentTrade;
        }
      }
